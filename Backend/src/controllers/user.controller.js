...
router.post('/checkout', protect, async (req, res) => {
  try {
    ...
    const updateInventory = store.inventory.map(
      ({ name, remaining, img, used, supplied, received }) => {
...
router.post('/checkout2', protect, async (req, res) => {
  try {
    let user = await User.findOne({ aadhaar: req.user.aadhaar });
    ...
    for await (const item of req.body) {
      let store = await Store.findById(item.store);
      ...
      const updateInventory = store.inventory.map(
        ({ name, remaining, img, used, supplied, received }) => {
          ...
          let value = 0;
          transaction.map((el) => {
            if (el.name === name && store._id.toString() === el.store) {
